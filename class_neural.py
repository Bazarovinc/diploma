...
from typing import List, NoReturn, Union  # импортирование библиотеки для типизации данных


# определение класса нейронной сети
class neuralNetwork:
    # инициализирование нейронной сети
    def __init__(self, input_nodes: int, hidden_nodes: List[int], output_nodes: int, learning_rate: float) -> NoReturn:
    ...
    def create_w(self) -> List[Union[np.ndarray, float]]:
        list_w = []
        # если длина скрытых слов 0, то мы создаем одну матрицу весов с размерам колличество выходных нейронов и входных
        if len(self.hnodes) == 0:
            # записываем в список весов сгенерированные случайным образом весовые коэффициенты с применением
            # нормального распределения и сдвигом -0.5
            list_w.append(np.random.normal(0.0, pow(self.onodes, -0.5), (self.onodes, self.inodes)))
        else:
            # иначе проходим по списку колличества нейронов скрытом слое и генерируем весовые коэффициенты
    ...
    def train(self, inputs_list: List[float], targets_list: List[float]) -> NoReturn:
