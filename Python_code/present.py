...
for _ in range(5):
    for i in range(len(input_new)):
        input_l = np.asfarray(input_new[i].split(','))  # превращаем строку в массив данных
        output_l = np.asfarray(output_data[i].split(',')[1:-1])  # превращаем строку в массив данных
...
ND = 15
Np = NS + NC + ND
UB = np.concatenate((0.01 * np.ones((NS, 1)), 0.4 * np.ones((4, 1)), 0.01 * np.ones((NC - 8, 1)), 0.4 * np.ones((4, 1)),
                              0.01 * np.ones((ND, 1))))
UB = UB.T[0]
final_input = np.concatenate(([0.2275], [0.1],	[0.3], UB))
